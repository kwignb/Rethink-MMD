...
from src.datasets.sampler import get_labels, MultiDataLoader, SamplingConfig
from src.datasets.office31 import DatasetAccess
...
class DatasetSizeType(Enum):
    ...
    def get_size(size_type, source_dataset, *other_datasets):
        if size_type == DatasetSizeType.Max.value:
            return max(list(map(len, other_datasets)) + [len(source_dataset)])
        elif size_type == DatasetSizeType.Source.value:
...
class DomainsDatasetBase:
    def prepare_data_loaders(self):
        """
        handles train/validation/test split to have 3 datasets each
        with data from all domains
        :return:
        """
    ...
    def get_domain_loaders(self, split="train", batch_size=32):
        ...
        Args:
            split (string, optional): ["train"|"valid"|"test"]. 
                                      Which dataset to iterate on. Defaults to "train".
            batch_size (int, optional): Defaults to 32.
        Returns:
            MultiDataLoader: A dataloader with API similar to the torch.dataloader,
                             but returning batches from several domains at each iteration.
        """
        raise NotImplementedError()
...
class MultiDomainDatasets(DomainsDatasetBase):
    def __init__(
        ...
        """
        The class controlling how the source and target domains are iterated over.
        Args:
    ...
    def prepare_data_loaders(self):
        ...
        if self._n_fewshot is not None and self._n_fewshot > 0:
            ...
            for part in ["train", "valid"]:
                self._labeled_target_by_split[part] = _split_dataset_few_shot(
                    self._target_by_split[part], self._n_fewshot
                )
            self._labeled_target_by_split["test"] = self._target_by_split["test"]
    ...
    def get_domain_loaders(self, split="train", batch_size=32):
        ...
        if self._labeled_target_by_split is None:
            ...
            # Few-shot supervised target domain
            ...
            ).create_loader(target_labeled_ds,
                            batch_size=min(len(target_labeled_ds), batch_size))
            
            n_dataset = DatasetSizeType.get_size(self._size_type,
                                                 source_ds,
                                                 target_labeled_ds)
            
            return MultiDataLoader(dataloaders=[source_loader, target_labeled_loader], 
                                   n_batches=max(n_dataset // batch_size, 1))
...
def _split_dataset_few_shot(dataset, n_fewshot, random_state=None):
    ...
    for class_ in classes:
        ...
        head, _ = np.split(indices, [n_fewshot])
